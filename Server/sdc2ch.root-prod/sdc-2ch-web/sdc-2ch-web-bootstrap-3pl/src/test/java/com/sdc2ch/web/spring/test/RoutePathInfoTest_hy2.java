...
public class RoutePathInfoTest_hy2 {
			...
			if(matrixs.isEmpty()) {
				...
				if(tmsplans.isEmpty()) {
							...
							if(last.getLat() == null) {
								...
								if(last.getStopCd().contains("47608")) {
									...
									y = 36.324742;
										...
										.lng(new BigDecimal(scale(last.getLng())).doubleValue()).name(last.getDlvyLoNm()).build();
							}
						}
					}
					end.setDate(utcFmtDate);
					...
					if(routeMap.get(key) == null) {
						...
						routeMap.put(matrix3.getStartPos() + matrix3.getEndPos(), matrix3);
					}else {
