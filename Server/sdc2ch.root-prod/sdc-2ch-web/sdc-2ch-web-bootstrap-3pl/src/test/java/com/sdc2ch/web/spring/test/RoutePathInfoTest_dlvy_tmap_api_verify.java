...
public class RoutePathInfoTest_dlvy_tmap_api_verify {
	...
	public void whenValidName_thenEmployeeShouldBeFound() throws IOException {
		...
		Stream.of("20181016_원장_검증").forEach(s -> {
			...
			Point point1 = Point.builder().lat(37.843731).lng(127.037366).name("신공장").build();
			
			
			infos.removeIf(i -> !i.getRouteNo().startsWith("2"));
			infos.forEach(i -> {
			...
			List<Point> points = Stream.of(point1).collect(Collectors.toList());
